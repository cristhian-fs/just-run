...
export function TimeValidation({ time, format }: { time: string, format: "short" | "long" }) {
  ...
  const getValidationMessage = (field: "minutes" | "seconds" | "hours") => {
    ...
    if (Number.isNaN(value) && format === "long") return timeMessage[format][field].required;
    if (Number.isNaN(value) && format === "short" && field !== "hours") return timeMessage.short[field].required;

    if ((field === "minutes" || field === "seconds") && (value < 0 || value > 59))
  ...
  const fields: ("hours" | "minutes" | "seconds")[] =
    format === "long" ? ["hours", "minutes", "seconds"] : ["minutes", "seconds"];
  ...
  return (
    ...
    >
      {fields.map((field) => {
        ...
        return (
          <div key={field} className="flex flex-col items-center gap-y-1 flex-1">
            <span
              className={cn("h-[2px] w-full rounded-full transition-colors bg-muted",
